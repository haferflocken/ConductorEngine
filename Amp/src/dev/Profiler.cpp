...
Collection::LinearBlockHashMap<uint64_t, Dev::Profiler::FrameRecordMap, Collection::I64HashFunctor> g_frameRecordsByRegistryID{ Collection::I64HashFunctor(), 4 };
...
void ProcessProfilerEventQueues()
{
	...
	for (auto&& entry : g_profilerRegistry.GetKeyValueView())
	{
		...
		Dev::Profiler::FrameRecordMap& frameRecordMap = *g_frameRecordsByRegistryID.Find(profiledThreadID);
...
uint64_t Dev::Profiler::PushFrame(const char* const name, const uint64_t beginNanoseconds)
{
...
void Dev::Profiler::PopFrame(const uint64_t endNanoseconds, const uint64_t previousFrameID)
{
	using namespace Internal_Profiler;
...
void Dev::Profiler::ForEachFrameRecordMap(const std::function<void(uint64_t, const FrameRecordMap&)>& fn)
{
	using namespace Internal_Profiler;
	std::unique_lock<std::mutex> lock{ g_profilerRegistryMutex };

	for (const auto& entry : g_frameRecordsByRegistryID.GetKeyValueView())
	{
...
FrameRecordMap::FrameRecord& FrameRecordMap::GetFrameRecord(uint64_t frameID)
{
	return const_cast<FrameRecord&>(static_cast<const FrameRecordMap*>(this)->GetFrameRecord(frameID));
}

const FrameRecordMap::FrameRecord& FrameRecordMap::GetFrameRecord(uint64_t frameID) const
{
	const size_t blockIndex = frameID / k_frameRecordBlockSize;
	const size_t indexInBlock = frameID % k_frameRecordBlockSize;
	const FrameRecordBlock& block = *m_frameRecordBlocks[blockIndex];
	return block[indexInBlock];
}

uint32_t FrameRecordMap::GetNumFrameRecords() const
{
	return (m_frameRecordBlocks.Size() - 1) * k_frameRecordBlockSize + m_numRecordsInLastBlock;
}

void FrameRecordMap::PushFrame(uint64_t frameID, uint64_t beginPoint, const char* name)
{
	...
	FrameRecord& frameRecord = CreateFrameRecord();
...
void FrameRecordMap::PopFrame(uint64_t frameID, uint64_t endPoint)
{
...
FrameRecordMap::FrameRecord& FrameRecordMap::CreateFrameRecord()
{
